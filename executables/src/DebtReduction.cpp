...
std::vector<::DebtChain> DebtNetwork::getSettleUpOptions(int creditorID) const
{
    // get all chains where the first creditor is the given creditor
    std::vector<::DebtChain> options;

    // queue of partial chains to loop over
    std::queue<DebtNetwork::DebtChain> partialChains;
    // first iteration of the loop will search for the inital debt to the given creditor
    for (const auto &debt : this->rootUser->getDebts())
    {
        if (debt.getCreditor()->userID == creditorID)
        {
            // start each partial reduction with the debt from the current user
            DebtNetwork::DebtChain partialChain;
            ...
            // this is itself a settle up option
            options.push_back(partialChain.getGeneric());
        }
    }

    // each iteration of this loop, pop off one partial chain to inspect it
    // while inspecting the chain, check if the chain may be extended, by appending the last creditor's creditors
    while (partialChains.size() > 0)
    {
        // remove the first partial chain for inspection
        auto partialChain = partialChains.front();
        ...
        {
            // call isInChain to prevent infinite loop
            if (!partialChain.isInChain(debt.getCreditor()))
            {
                // copy the chain to add the link
                auto partialChainCopy = partialChain;
                // add this creditor to the chain
                partialChainCopy.addLink(debt.getCreditor(), lastCreditor, debt.getAmount());

                // this is now another settle up option
                options.push_back(partialChainCopy.getGeneric());

                // only continue searching this chain if it's not already the maximum size
                if (partialChain.numLinks() < DebtNetwork::DebtChain::MAX_CHAIN_LINKS)
                {
                    partialChains.push(partialChainCopy);
                }
            }
        }
    }
